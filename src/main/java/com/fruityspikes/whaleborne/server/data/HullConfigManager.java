...
/**
 * Loads per-material hull configurations from datapack JSON files at
 * data/{namespace}/whaleborne/hull_config/{name}.json (platforms only, no seat layouts).
 */
public class HullConfigManager extends SimpleJsonResourceReloadListener {
    ...
    protected void apply(Map<ResourceLocation, JsonElement> entries, ResourceManager resourceManager, ProfilerFiller profiler) {
        ...
        Whaleborne.LOGGER.info("Loaded {} hull configs ({} with custom platforms, {} with custom seats)",
                CONFIGS.size(), PLATFORM_LAYOUTS.size(), SEAT_LAYOUTS.size());
    }
    ...
    public static HullConfig getConfig(Item item) {
        ...
        if (item instanceof BlockItem blockItem) {
            try {
                ...
                if (destroySpeed > 0) resistance = destroySpeed;
            } catch (Exception e) {
                // Modded block threw — keep default resistance
            }
        }
    ...
    public static int getMaxPlanks(Item item) { return getConfig(item).planksRequired(); }
    public static float getResistance(Item item) { return getConfig(item).resistance(); }
    public static String getArmorModel(Item item) { return getConfig(item).armorModel(); }
    public static float getBlockChance(Item item) { return getConfig(item).getEffectiveBlockChance(); }
    public static float getSwimSpeedBonus(Item item) { return getConfig(item).swimSpeedBonus(); }
