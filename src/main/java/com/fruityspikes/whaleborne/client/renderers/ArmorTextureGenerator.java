...
/**
 * Auto-generates armor textures for modded wood via delta-based palette recoloring,
 * preserving template detail while shifting colors toward the target wood's palette.
 */
public class ArmorTextureGenerator {
    private static final Logger LOGGER = LoggerFactory.getLogger("Whaleborne-ArmorTexGen");

    private static final ResourceLocation OAK_ARMOR_TEMPLATE = new ResourceLocation(
            Whaleborne.MODID, "textures/entity/armor/hullback_oak_planks_armor.png");
    private static final ResourceLocation OAK_BLOCK_TEXTURE = new ResourceLocation(
            "minecraft", "textures/block/oak_planks.png");

    private static final Map<Item, CachedGenerated> CACHE = new HashMap<>();
    private static List<float[]> oakBlockPalette = null;
    ...
    public static ResourceLocation getOrGenerateArmorTexture(Item item) {
        ...
        try {
            ...
            ResourceLocation texLoc = new ResourceLocation(Whaleborne.MODID, texName);
            ...
            LOGGER.info("Auto-generated armor texture for {} ({} palette colors)", itemId, targetPalette.size());
            return texLoc;
        } catch (Exception e) {
    ...
    public static void clearCache() {
        for (CachedGenerated entry : CACHE.values()) entry.texture.close();
        CACHE.clear();
            ...
            try (InputStream in = res.get().open()) {
                ...
                for (int y = 0; y < h; y++) {
                    for (int x = 0; x < w; x++) {
                        ...
                        if ((pg > pr * 1.03f) && psat > 0.10f) continue;
                ...
                for (int y = 0; y < h; y++) {
                    for (int x = 0; x < w; x++) {
                        ...
                        float t = armorLumRange > 0.001f
                                ? Math.max(0f, Math.min(1f, (pixLum - armorMinLum) / armorLumRange))
                                : 0.5f;
    ...
    /**
     * Looks up a block's particle-icon texture; works for any modded wood with a sprite atlas entry.
     */
    private static ResourceLocation getBlockTexturePath(BlockItem blockItem) {
        try {
            ...
            return new ResourceLocation(spriteName.getNamespace(),
