...
/**
 * Loads armor piece geometry from datapack JSON (per-material override, else default).
 * Kept outside {@code models/} so vanilla {@code ModelManager} won't scan it.
 */
public class ArmorModelLoader {
    ...
    public static LayerDefinition loadPiece(String piece, String material) {
        if (material != null) {
            LayerDefinition result = tryLoadJson(GEO_BASE + material + "/" + piece + ".json", piece);
    ...
    private static LayerDefinition tryLoadJson(String path, String pieceName) {
        ResourceLocation loc = new ResourceLocation(Whaleborne.MODID, path);
        try {
    ...
    /** Loads all three armor pieces (head/body/fluke) into a single LayerDefinition. */
