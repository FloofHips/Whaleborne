...
/**
 * Renders code-generated water wake effects for the Hullback via a custom no-depth-write
 * RenderType, so foam overlays water without cutting it.
 */
public class HullbackWakeRenderer {
    ...
    public static boolean isWakesModLoaded() {
        if (wakesModLoaded == null) {
            wakesModLoaded = net.minecraftforge.fml.ModList.get().isLoaded("wakes");
        }
    ...
    /**
     * Renders the bow wake as a U-shaped foam pattern (front bar + side strips), each with
     * a wider water-displacement layer and a narrower near-white crest layer above it.
     */
    ...
    /**
     * Renders a dual-layer foam ribbon from historical tail positions: a bright narrow
     * crest over a wide diffuse spread, with sine width-pulsing and UV current flow.
     */
    ...
    /**
     * Generates a 64x64 foam texture of 4x4 white cells on a transparent background.
     */
