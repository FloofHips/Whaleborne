...
public final class WakesCompat {
    ...
    public static void generatePartWakes(HullbackEntity hullback) {
        ...
        Vec3[] partPositions = hullback.getPartPositionsArray();
        if (partPositions == null) return;
        ...
        int phase = tick & 1;
        int partCount = Math.min(partPositions.length, PART_COUNT);

        cachedFwdValid = false;

        for (int i = 0; i < partCount; i++) {
            Vec3 pp = partPositions[i];
            if (pp == null) continue;
            ...
            if (i == 1 && partPositions[0] != null) {
                double fdx = partPositions[0].x - cx;
                double fdz = partPositions[0].z - cz;
        ...
        if (hullback.wakeIntensity >= BOW_SPLASH_MIN_INTENSITY
                ...
                && cachedFwdValid) {
            spawnBowSplash(hullback, partPositions, waterY, armored);
        }
    ...
    private static void spawnBowSplash(HullbackEntity hullback, Vec3[] partPositions,
                                       float waterY, boolean armored) {
        Vec3 nosePos = partPositions[0];
